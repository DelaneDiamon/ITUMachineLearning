...
head(wine_data)
observation_num<-nrow(wine_data)
wine_data_sample<-wine_data[sample(observation_num, observation_num, replace = FALSE),]
k =3
wine_train = matrix(0, (k-1)*observation_num/k, ncol(wine_data))
wine_train = wine_data_sample[1:(observation_num*(2/3)),]
max_depth=10
...
for(idepth in 1:max_depth){
  ...
  for(j in 1:k){
    ...
    for (i in 1:k){
      if(i == pick){
        wine_cross = wine_data_sample[((i-1)*observation_num/k+1):(observation_num*(i/k)),]
      } else {
        wine_train[((i_tmp-1)*observation_num/k+1):(observation_num*(i_tmp/k)),] = wine_data_sample[((i-1)*observation_num/k+1):(observation_num*(i/k)),]
        i_tmp = i_tmp +1
      }
    }
    pick = pick -1
    y_wine = wine_train[,12]
    x_wine = wine_train[,1:11]
    fit<-rpart(y_wine~., x_wine, control = rpart.control(maxdepth = idepth))
    errTrain = sqrt(sum((y_wine-predict(fit,x_wine))^2)/length(y_wine))
    trainErrTot[idepth] = trainErrTot[idepth] + errTrain
    
    y_wine_cross = wine_cross[,12]
    x_wine_cross = wine_data[,1:11]
    
    crossError = sqrt(sum((y_wine_cross-predict(fit, x_wine_cross))^2)/length(y_wine_cross))
...
plot_frame = data.frame(wine_data_sample$alcohol, wine_data_sample$quality)

correlation_wine = abs(cor(wine_data_sample[1,1:11], wine_data_sample$quality))
correlation_wine

plot(cor_wine[,1], xaxt = "n")
axis(1, at=1:11, labels = row.names((correlation_wine)))

plot_frame2 = data.frame(wine_data_sample$volatile.acidity, wine_data_sample$quality)
plot(plot_frame2)
plot(plot_frame)
@
