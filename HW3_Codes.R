...
wine_test_fit<-predict(lm_wine_train, newdata = as.data.frame(x_wine_test))
...
# QUESTION 2
# Perform a ridge regression on the wine quality data set from problem 1 using only the first 1400 observations.
...
test_wine_ridge = wine_data[1401:dim(scaled_wine_data)[1],]
...
predictedQuality= predict(ridgeMod, newx = as.matrix(test_x_wine))
ridge_error = sqrt(sum((test_y_wine - predictedQuality)^2))/length(predictedQuality)
ridge_coef<-coef(ridgeMod)

paste("using Ridge the estimated test error = ", ridge_error)
